...
func (m UserModel) Insert(user *User) error {
	...
	defer cancel()
	err := m.DB.QueryRowContext(ctx, sqlQuery, args...).Scan(&user.ID, &user.Created_at, &user.Version)
	if err != nil {
		switch {
...
func (m UserModel) GetByEmail(email string) (*User, error) {
	sqlQuery := `SELECT id,created_at,name,email,password_hash,activated,version
					FROM users
					WHERE email=$1`
...
func (m UserModel) Update(user *User) error {
	...
	err := m.DB.QueryRowContext(ctx, sqlQuery, args...).Scan(&user.Version)
	if err != nil {
		switch {
		case err.Error() == `pq: duplicate key value violates unique constraint "users_email_key"`:
			return ErrDuplicateEmail
